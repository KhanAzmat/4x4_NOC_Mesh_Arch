...
int test_c0_gather(mesh_platform_t *p)
{
    ...
    for (int d = 0; d < 8; ++d) {
        ...
        thread_safe_printf("%d. HAL transfer: dmem_%d(0x%lx) -> node_0.dlm1+%d(0x%lx)\n", 
    ...
    thread_safe_printf("[C0-Gather] Summary: %d/8 passed\n\n", pass);
    return pass == 8;
}
...
int test_c0_distribute(mesh_platform_t *p)
{
    ...
    for (int d = 0; d < 8; ++d) {
        ...
        thread_safe_printf("%d. HAL transfer: node_0.dlm1(0x%lx) -> dmem_%d(0x%lx), size: %d\n", 
    ...
    thread_safe_printf("[C0-Distribute] Summary: %d/8 passed\n\n", pass);
    return pass == 8;
}
